...
{/******************************* New functions to handle button calls for feature ********************************/}
	

function handleRecentVolunteers(){

	const pastVolunteers = document.getElementById('pastVolunteersButton')
	const collection = document.getElementsByClassName("editControls")

	for (let i = 0; i < collection.length; i++)
      	  collection[i].style.display = 'none'
    		pastVolunteers.style.display = 'inline'
    		pastVolunteers.innerHTML = "List Volunteers in the Past 2 Days"
	...
	axios.get(`${process.env.REACT_APP_HOST}/api/read`).then((response) => {
		...
		for (var i = 0; i < arr.length; i++) {
        		console.log(arr[i]);
        		document.getElementById("displayData").innerHTML += arr[i].first_name + " " + arr[i].last_name +  "<br />"
    		}
		  //document.getElementById("displayData").innerHTML = arr[0].first_name
    	})

}


function volunteersByDate(){
...
{/************************************************ New feature buttons ********************************************/}
	...
	<button id="pastVolunteersButton" onClick={handleRecentVolunteers}>List Volunteers in Past 2 Days</button>
	<button id="volunteersByDate" onClick={volunteersByDate}>List Volunteers by Date</button>
