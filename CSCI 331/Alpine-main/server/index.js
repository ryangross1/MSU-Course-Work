...
const db = mysql.createPool({ // createConnection
    //host: 'localhost',
    host: 'http://csci331.cs.montana.edu',
...
app.post("/api/create", (req, res) => {
    ...
    const sqlInsert = "INSERT INTO volunteers (first_name, last_name, email_address, date_in) VALUES (?,?,?,?);"
    db.query(sqlInsert, [fn, ln, ea, dt], (err, result) => {
        if(err) throw err
...
app.get("/api/lastcouple", (req, res) => {
    const sqlSelect = "SELECT * FROM volunteers;"
    db.query(sqlSelect, (err, result) => {
        if(err){
            throw err;
